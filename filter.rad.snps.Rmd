...
samps<-samps[match(colnames(vcfR@gt)[-1], samps$id),]
...
#looks like 3 samples that just don't improve and will always be low
#run function to drop samples above the threshold we want from the vcf
vcfR<-missing_by_sample(vcfR=vcfR, cutoff = .7)
...
#mac filter significantly improves our ability to discrimate between clusters, i.e., increases signal to noise ratio, so we will keep this one for downstream analyses
```
...
#write.vcf(vcf.filt, file = "~/Desktop/hipposideros/filtered.vcf.gz")

#filter to one SNP per locus to get an unlinked dataset
unlinked.vcf<-distance_thin(vcf.filt, min.distance = 10000)
